...
export class HotLoader {
  ...
  async reload() {
    ...
    // 2) config.json（热阈值）
    let cfg = { ...HOT_CONFIG_DEFAULTS }
    try {
      const raw = JSON.parse(await fs.readFile(this.configFile, 'utf8'))
      if (typeof raw.blockThreshold === 'number') cfg.blockThreshold = raw.blockThreshold
      if (typeof raw.warnThreshold === 'number') cfg.warnThreshold = raw.warnThreshold
    } catch { /* 无配置文件用默认 */ }
    ...
    this.status = {
      ...
      customRules: customInput.length + customOutput.length,
