...
// 五个维度加权：
//   维护活跃度 30%（GitHub stars / 最近提交 / 更新时间 —— 可实时查询，失败给中性分）
//   代码质量   20%（类型声明 / 测试 / lint / 文件规模）
//   已知漏洞   20%（npm audit 结果，无 audit 则看依赖复杂度）
//   来源可信   15%（官方 @deepseek-ai / 已收录 awesome 目录 / 未知）
//   运行时健康 15%（依赖完整性）
// 总分 ≥78 A · 70-77 B · 50-69 C · <50 D
// D 级 = 门禁拒绝安装（默认）。
...
function scoreQuality(manifest, files) {
  ...
  if (files.some((f) => /(^|\/)(?:eslint\.config\.|\.eslintrc|\.oxlintrc)/.test(f))) { s += 5; notes.push('含 lint 配置') }
