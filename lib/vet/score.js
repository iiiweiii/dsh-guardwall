...
export const GRADE_LIMITS = { A: 85, B: 70, C: 50, D: 0 }
 ...
 */
export async function scorePackage({ manifest, files, permissions, staticScan, githubMeta, dir }) {
  const repoRef = (() => {
    const r = manifest.repository || ''
    const m = r.match(/github\.com[/:]([\w.-]+\/[\w.-]+)/)
    return m ? m[1] : null
  })()
  ...
  // 权限与静态发现的降级（从总分扣，封顶 -30）
  let penality = 0
  const findings = staticScan?.findings || []
  if ((permissions?.maxSeverity || 0) >= 8) penality += 15
  if ((staticScan?.maxSeverity || 0) >= 8) penality += 10
  if (findings.length >= 50) penality += 5
  if (findings.some((f) => f.defensive)) penality -= 2 // 有防御代码的反而体现安全意识
  penality = Math.max(-3, Math.min(penality, 30))
