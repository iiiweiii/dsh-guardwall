...
export async function runNpmAudit(dir) {
  try {
    const { stdout } = await runNpm(['audit', '--json', '--omit=dev'], {
      cwd: dir, timeout: 30000, allowNonZero: true,
    })
    // npm audit intentionally exits non-zero when vulnerabilities are found;
    // stdout is still the authoritative JSON report in that case.
...
/** 汇总全部静态发现 */
export async function fullStaticScan({ manifest, files, sources, auditDir = null }) {
  const findings = []
  ...
  // 有 lockfile 才跑 audit
  const hasLock = Boolean(auditDir) || files.includes('package-lock.json') || files.includes('pnpm-lock.yaml') || files.includes('yarn.lock')
  let audit = null
  if (hasLock && (auditDir || manifest.root)) {
    try { audit = await runNpmAudit(auditDir || manifest.root) } catch { /* 忽略 */ }
  }
  return {
